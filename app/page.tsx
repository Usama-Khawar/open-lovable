...
export default function HomePage() {
  ...
  const handleSubmit = async (selectedResult?: SearchResult) => {
    ...
    console.log("[handleSubmit] Called with:", { inputValue, selectedResult });
    ...
    if (isURL(inputValue)) {
      ...
      // It's a text description - use directly as prompt for AI generation
      console.log(
        "[handleSubmit] Text prompt mode, redirecting to generation..."
      );
      sessionStorage.setItem("userPrompt", inputValue);
      sessionStorage.setItem("selectedStyle", selectedStyle);
      sessionStorage.setItem("selectedModel", selectedModel);
      sessionStorage.setItem("autoStart", "true");
      console.log("[handleSubmit] SessionStorage set, calling router.push");
      router.push("/generation");
      console.log("[handleSubmit] router.push called");
    }
  };

  // Search functionality disabled (no web scraping)
    ...
    <HeaderProvider>
      <div className="min-h-screen bg-background-base">
        ...
        <div className="sticky top-0 left-0 w-full z-[101] bg-background-base header">
          ...
          <HeaderWrapper>
            <div className="max-w-[900px] mx-auto w-full flex justify-between items-center"></div>
          </HeaderWrapper>
        </div>
                    ...
                    {hasSearched && searchResults.length > 0 && !isFadingOut ? (
                      <>
                        ...
                        <div className="flex-1 text-body-input text-accent-black">
                          Select a result to use as inspiration for your website
                        </div>
                      ...
                      <>
                        ...
                        >
                          <HeroInputSubmitButton
                            dirty={url.length > 0}
                            buttonText={isURL(url) ? "Generate" : "Generate"}
                            disabled={isSearching}
                          />
                        </div>
                      </>
