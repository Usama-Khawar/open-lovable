...
function AISandboxPage() {
  ...
  const sandboxRecreatingRef = useRef<boolean>(false);
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      if (storedPrompt) {
        // User entered a text description (not a URL)
        setHasInitialSubmission(true);

        // Don't clear sessionStorage yet - let the auto-start effect handle it
        ...
        // Auto-start flag will be checked by the useEffect after sandbox is created
        // The autoStart flag is already set by home page
      } else if (storedUrl) {
      ...
      try {
        ...
        // If we have a URL or prompt from the home page, mark for automatic start
        if ((storedUrl || storedPrompt) && isMounted) {
          // We'll trigger the generation after the component is fully mounted
          // and the startGeneration function is defined
          // Note: autoStart flag should already be set by home page, but ensure it's there
  ...
  // Auto-start generation if flagged (for both URL and text prompts)
  useEffect(() => {
    ...
    if (autoStart === "true" && !showHomeScreen && hasPromptOrUrl && sandboxData) {
      ...
      setTimeout(() => {
        console.log(
          "[generation] Auto-starting generation with:",
          homeUrlInput ? `URL: ${homeUrlInput}` : `Prompt: ${homeContextInput}`
        );
        startGeneration();
      }, 1500);
    }
  }, [showHomeScreen, homeUrlInput, homeContextInput, sandboxData]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Start keep-alive when sandbox is created
  useEffect(() => {
    if (sandboxData && sandboxData.sandboxId) {
      console.log("[keepalive] Sandbox created, starting keep-alive");
  ...
  }, [sandboxData?.sandboxId]); // eslint-disable-line react-hooks/exhaustive-deps

  // Monitor iframe for 502/410 errors and auto-fix
  useEffect(() => {
    ...
    const checkIframeStatus = async () => {
      ...
      try {
        // Try to fetch the sandbox URL to check if it's responding
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 5000);

        const response = await fetch(sandboxData.url, {
          method: "HEAD",
    ...
    const intervalId = setInterval(checkIframeStatus, 30000);

    return () => {
      clearInterval(intervalId);
    };
  }, [sandboxData?.url]); // eslint-disable-line react-hooks/exhaustive-deps
...
Tip: I automatically detect and install npm packages from your code imports (like react-router-dom, axios, etc.)`,
  ...
  // Sandbox keep-alive mechanism to prevent 410 errors
  const startSandboxKeepAlive = () => {
  ...
  const stopSandboxKeepAlive = () => {
    if (keepAliveIntervalRef.current) {
      clearInterval(keepAliveIntervalRef.current);
      keepAliveIntervalRef.current = null;
      console.log("[keepalive] Stopped sandbox keep-alive pings");
    }
  };

  // Check and restart Vite if needed
  const checkAndRestartVite = async () => {
    if (!sandboxData) {
      console.log("[vite-check] No sandbox data available");
      return;
    }

    console.log("[vite-check] Checking Vite status...");
    ...
    try {
      ...
      console.error("[vite-check] Failed to restart Vite:", error);
      addChatMessage(
        "⚠️ Failed to restart Vite. The sandbox might need recreation.",
        "system"
      );
    }
  };

  // Handle sandbox timeout or 410 errors
  const handleSandboxTimeout = async () => {
    if (sandboxRecreatingRef.current) {
      console.log("[sandbox-timeout] Already recreating, skipping...");
      return;
    }

    sandboxRecreatingRef.current = true;
    console.log("[sandbox-timeout] Handling sandbox timeout, will recreate...");

    addChatMessage(
      "⚠️ Sandbox session expired. Creating a new sandbox...",
      "system"
    );
    ...
    try {
      ...
      console.error("[sandbox-timeout] Failed to recreate sandbox:", error);
      addChatMessage(
        `❌ Failed to recreate sandbox: ${error.message}. Please refresh the page.`,
        "error"
      );
    } finally {
      sandboxRecreatingRef.current = false;
    }
  };
        ...
        return (
          <div className="relative w-full h-full">
            <iframe
              ...
              onError={(e) => {
                console.error("[iframe] Error loading sandbox:", e);
                // Try to restart Vite when iframe fails to load
                checkAndRestartVite();
              }}
            />
  ...
  const captureUrlScreenshot = async (url: string) => {
    // Screenshot capture disabled (no web scraping)
  ...
  const startGeneration = async () => {
    // Allow generation with either URL or text prompt
    if (!homeUrlInput.trim() && !homeContextInput.trim()) return;
    ...
    const hasUrl = homeUrlInput.trim().length > 0;
    ...
    if (hasUrl) {
      ...
      // Remove protocol for cleaner display
      cleanUrl = displayUrl.replace(/^https?:\/\//i, "");
    }
    ...
    addChatMessage(
      hasUrl
        ? brandExtensionMode
          ...
          : `Starting to build from ${cleanUrl}...`
        : "Starting to generate your website...",
      "system"
    );
    ...
    // Only capture screenshot if there's a URL
    if (hasUrl) {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (hasUrl) {
          // User provided a URL as reference
          setUrlStatus(["Processing URL reference..."]);
          ...
          addChatMessage(
            `Using ${cleanUrl} as reference for the design...`,
            "system"
          );
          scrapeData = {
            success: true,
          ...
          // User provided a text prompt only
          setUrlStatus(["Processing your request..."]);
          addChatMessage(
            "Generating website from your description...",
            "system"
          );
        }

        setUrlStatus(["Ready to generate!", "Building your app..."]);
        ...
        if (brandExtensionMode && brandGuidelines) {
          ...
          setConversationContext((prev) => ({
            ...
            currentProject: `Custom build using ${homeUrlInput} brand`,
          }));
...
BRAND GUIDELINES FROM ${homeUrlInput}:
...
TYPOGRAPHY:
- Primary Font: ${
            branding.typography?.fontFamilies?.primary || "system default"
          }
- Heading Font: ${
            branding.typography?.fontFamilies?.heading || "system default"
          }
- Font Stack (Body): ${
            branding.typography?.fontStacks?.body?.join(", ") ||
            "system-ui, sans-serif"
          }
- Font Stack (Heading): ${
            branding.typography?.fontStacks?.heading?.join(", ") ||
            "system-ui, sans-serif"
          }
...
Primary Button:
  - Background: ${
    branding.components?.buttonPrimary?.background || branding.colors?.primary
  }
  - Text Color: ${branding.components?.buttonPrimary?.textColor || "#FFFFFF"}
  - Border Radius: ${
    branding.components?.buttonPrimary?.borderRadius ||
...
Secondary Button:
  ...
  - Border Radius: ${
    branding.components?.buttonSecondary?.borderRadius ||
    branding.spacing?.borderRadius ||
    "8px"
  }
...
- Border Radius: ${
            branding.components?.input?.borderRadius ||
            branding.spacing?.borderRadius ||
            "6px"
          }
...
${
  branding.images?.logo
    ? `- Logo Available: Yes (use carefully if needed)`
    : "- Logo: Not available"
}
...
<branding-format source="${cleanUrl}">
${brandInstructions}
...
${
  homeContextInput ||
  "Build a modern web component using these brand guidelines"
}

IMPORTANT: The content above in the <branding-format> tags contains the extracted brand guidelines from ${cleanUrl}.
Use these guidelines (colors, fonts, spacing, design patterns) to build what the user requested.

CRITICAL REQUIREMENTS:
- DO NOT recreate the original website at ${cleanUrl}
- DO create a COMPLETELY NEW component that fulfills the user's request
- The user wants: "${homeContextInput}"
...
- Use the specified border radius (${
            branding.spacing?.borderRadius || "6px"
          }) consistently
...
- Apply the brand personality: ${
            branding.personality?.tone || "professional"
          } tone with ${branding.personality?.energy || "medium"} energy
...
Focus on building something NEW, minimal, and functional that perfectly matches the ${
            brandGuidelines.styleName || "brand"
          } aesthetic and design system.`;
        } else if (hasUrl && scrapeData) {
          // === URL REFERENCE MODE PROMPT ===
          ...
          setConversationContext((prev) => ({
            ...
            currentProject: `Website inspired by ${cleanUrl}`,
          }));
          ...
          prompt = `I want to build a website inspired by ${cleanUrl} as a complete React application.
...
Please build a website that follows these requirements.`
    : ""
}

IMPORTANT INSTRUCTIONS:
- Create a COMPLETE, working React application
- Use the reference URL as inspiration for design and layout
- Use Tailwind CSS for all styling (no custom CSS files)
...
${
  filteredContext
    ? "- Apply the user's requirements throughout the application"
    : ""
}

Focus on creating a clean, modern website.`;
        } else {
          // === TEXT PROMPT ONLY MODE ===
          setConversationContext((prev) => ({
            ...prev,
            currentProject: `Custom website build`,
          }));

          const userDescription = homeContextInput.trim();

          prompt = `I want you to build a complete React application based on this description:
...
- Use Tailwind CSS for all styling (no custom CSS files)
- Make it responsive and modern
- Create proper component structure with multiple components if needed
- Make sure the app actually renders visible content immediately
- Create ALL components that you reference in imports
- Focus on clean, modern design
- Include all necessary sections and features mentioned in the description

Build a professional, polished website that fulfills the user's request.`;
        }
        ...
        if (generatedCode) {
          ...
          addChatMessage(
            brandExtensionMode
              ...
              : `Successfully recreated ${homeUrlInput} as a modern React app${
                  homeContextInput
  ...
  return (
    <HeaderProvider>
      <div className="font-sans bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 text-gray-100 h-screen flex flex-col">
        {/* Premium Header Bar */}
        <div className="bg-black/40 backdrop-blur-xl border-b border-white/10 px-6 py-3 flex items-center justify-between shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            {sandboxData?.sandboxId && (
              <span className="text-xs text-gray-400 font-mono px-3 py-1 bg-white/5 rounded-lg border border-white/10">
                {sandboxData.sandboxId.slice(0, 8)}...
              </span>
            )}
            {status.active && (
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 max-w-[420px] flex flex-col border-r border-white/10 bg-gradient-to-b from-gray-900/50 to-gray-900/30 backdrop-blur-sm">
                      ...
                      <div key={idx} className="flex flex-col gap-3">
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          <a
                            ...
                            className="text-gray-100 hover:text-orange-400 truncate max-w-[250px] font-medium transition-colors"
                            title={sourceURL}
                          >
                            {siteName}
                          </a>
                              ...
                              >
                                <svg
                                  ...
                                  className={`transition-transform duration-300 ${
                                    screenshotCollapsed ? "rotate-180" : ""
                                  }`}
                                >
                                  <path
                                    d="M4 6L8 10L12 6"
            ...
            <div
              className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 scrollbar-hide bg-gradient-to-b from-transparent to-gray-900/20"
              ref={chatMessagesRef}
            >
                ...
                return (
                  <div key={idx} className="block">
                    ...
                    >
                      <div className="block">
                        <div
                          className={`block rounded-xl px-4 py-3 shadow-lg ${
                            msg.type === "user"
                              ? "bg-gradient-to-br from-orange-500 to-orange-600 text-white ml-auto max-w-[80%] shadow-orange-500/20"
                              : msg.type === "ai"
                              ? "bg-white/10 backdrop-blur-md border border-white/20 text-gray-100 mr-auto max-w-[80%]"
                              : msg.type === "system"
                              ? "bg-blue-500/10 border border-blue-500/20 backdrop-blur-md text-blue-300 text-sm"
                              : msg.type === "command"
                              ? "bg-gray-800/50 border border-gray-700/50 backdrop-blur-md text-gray-300 font-mono text-sm"
                              : msg.type === "error"
                              ? "bg-red-500/10 border border-red-500/30 backdrop-blur-md text-red-300 text-sm"
                              : "bg-gray-800/50 border border-gray-700/50 backdrop-blur-md text-gray-300 text-sm"
                          }`}
                        >
                          {msg.type === "command" ? (
            ...
            <div className="p-4 border-t border-white/10 bg-gray-900/50 backdrop-blur-sm">
              <HeroInput
                ...
                placeholder="Describe what you want to build or edit..."
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="px-6 pt-4 pb-4 bg-black/20 backdrop-blur-xl border-b border-white/10 flex justify-between items-center">
              <div className="flex items-center gap-2">
                {/* Toggle-style Code/View switcher */}
                <div className="inline-flex bg-white/5 backdrop-blur-md border border-white/10 rounded-lg p-1">
                  <button
